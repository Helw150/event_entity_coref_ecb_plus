...
def get_mention_span_rep(mention, device, model, docs, is_event,
    ...
    # if not is_event:
    #     global next_number, mention_mapping
    #     mention_id = mention.gold_tag
    #     if mention_id in mention_mapping:
    #         mention_number = mention_mapping[mention_id]
    #     else:
    #         next_number += 1.0
    #         mention_mapping[mention_id] = next_number
    #         mention_number = next_number
    #     span_tensor = model.ent_embed(
    #         torch.tensor(mention_number,
...
def merge(clusters, cluster_pairs, other_clusters, model, device, topic_docs,
    ...
    for cluster_1 in clusters:
        ...
        for cluster_2 in clusters:
            if cluster_1 == cluster_2:
                ...
                pair_score = assign_score(
                    (cluster_1, cluster_2), model, device, topic_docs,
                    is_event, use_args_feats, use_binary_feats, other_clusters)
                row.append(1 - pair_score)
        distance_list.append(row)

    clusterer = hdbscan.HDBSCAN(min_cluster_size=2, metric='precomputed')
    clusterer.fit(np.array(distance_list))
    print(clusterer.labels_)

    clusters_dict = {}
    for cluster, cluster_id in zip(clusters, clusterer.labels_):
        if cluster_id not in clusters_dict:
            clusters_dict[cluster_id] = [cluster]
        else:
            clusters_dict[cluster_id].append(cluster)

    for cluster_mention in clusters_dict:
        if cluster_mention != -1:
            seed = None
            for cluster in clusters_dict[cluster_mention]:
                if seed:
                    print('epoch {} topic {}/{} - merge {} clusters : {} {}'.
                          format(epoch, topics_counter, topics_num, mode,
                                 str(seed), str(cluster)))
                    logging.info(
                        'epoch {} topic {}/{} - merge {} clusters : {} {}'.
                        format(epoch, topics_counter, topics_num, mode,
                               str(seed), str(cluster)))
                    merge_clusters((seed, cluster), clusters, other_clusters,
                                   is_event, model, device, topic_docs,
                                   pairs_dict, use_args_feats,
                                   use_binary_feats)
                    seed = clusters[-1]
                else:
                    seed = cluster
    ...
    if -1 in clusters_dict:
        logging.info("{} singletons for epoch {} topic {}/{}".format(
            str(len(clusters_dict[-1])), epoch, topics_counter, topics_num))

    print("Finished DBSCAN Clustering for epoch {} topic {}/{}".format(
        epoch, topics_counter, topics_num, mode))
    # for pair in cluster_pairs:
    #     pair_score = assign_score(pair, model, device, topic_docs, is_event,
    #                               use_args_feats, use_binary_feats,
    #                               other_clusters)
